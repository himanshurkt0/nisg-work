...
<script src="<c:url value='/assets/js/jquery-1.12.4.min.js'/>" type="text/javascript"></script>
 <script type="text/javascript">
	...
	function callController(appName,parentId,schemaName,appCalled){
		$.ajax({
            url: 'secondLevelHomeController',
            type: 'POST',
...
.sub-item ul li{text-align:left; border-bottom:1px solid rgba(255, 255, 255, 0.5);overflow-y: auto;} 
...
<body id="page-top">
  ...
  <div id="wrapper" >
    ...
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="home.html" style = "height:50px; width:110px">
     ...
     <li class="nav-item">
     	<a class="nav-link" href = "<c:url value='/afterLogin'/>">
     	 	<em></em> &nbsp; 
     		<span>Go Back</span>
     	</a>
     </li>
     	
		<c:forEach var="entry" items="${schemaAppMap}">
			<li class="nav-item" >
				<a class="nav-link"  href="<c:url value='/secondLevelHome/${appName}/${schemaName}/${entry.value}/${entry.key}'/>">
          				<em></em> &nbsp; 
          				<span>${entry.key}</span>
        		</a> 
			</li>
      ...
      <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light topbar mb-2 static-top shadow" style="background-color: #146C94 !important;">
         ...
         <div class="d-none d-sm-inline-block form-inline mr-auto my-2 my-md-0 mw-100 navbar-search">
          <h4 class="text-white"><strong><spring:message code="app.lang.HomeTitle"/>${appNameTracker }</strong></h4>   
         </div>
